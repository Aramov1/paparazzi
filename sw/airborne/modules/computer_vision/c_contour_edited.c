...
void find_contour(char *img, int width, int height)
{
  ...
  /* blob Y centroids — ensure top (smaller Y) and bottom (larger Y) ordering */
  cont_est.top_cy = (top_cy < bottom_cy) ? (int)top_cy : (int)bottom_cy;
  cont_est.bot_cy = (top_cy < bottom_cy) ? (int)bottom_cy : (int)top_cy;
